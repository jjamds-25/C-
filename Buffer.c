...
int bufferSocketRead(struct Buffer* buffer, int fd)
{
    ...
    // readv的参数 (int fd, const struct iovec *iov, int iovcnt)
    // fd : 用于通信的文件描述符
    // iov : struct iovec结构体数组的一个实例化
    // iovcnt : 实例化几个iovec结构体元素
...
{
    // strstr --> 大字符串中匹配子字符串(遇到\0结束) char *strstr(const char *haystack, const char *needle);返回值是匹配到的字符串首元素地址
