...
void taskWakeup(struct EventLoop* evLoop)
{
    ...
    // 往socketPair[0]里写数据，那么socketPair[1]就被激活了，读回调readLocalMessage就会被调用。
...
struct EventLoop* eventLoopInitEx(const char* threadName)
{
    ...
    // socketPair的参数（int domain, int type, int protocol, int sv[2]）
    // domain : 与socket函数的第一个参数一样,设置通信方式，如AF_INET,AF_LOCAL
    // type : 与socket函数的第二个参数一样,设置通信协议，如SOCK_STREAM,SOCK_DGRAM
    // protocol : 与socket函数的第三个参数一样,tcp or udp,设置为0
    // sv[2] : 传出参数，需要传一个有两个元素的数组的地址，sv[0]和sv[1]之间可以通信
